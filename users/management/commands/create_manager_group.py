...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        recipient_perms = Permission.objects.filter(codename__in=["can_view_all_recipients"])
        managers.permissions.add(*recipient_perms)

        # Добавление прав для модели Message
        message_perms = Permission.objects.filter(codename__in=["can_view_all_messages"])
        managers.permissions.add(*message_perms)

        # Добавление прав для модели Mailing
        mailing_perms = Permission.objects.filter(codename__in=["can_view_all_mailings", "can_deactivate_mailing"])
        ...
        if created:
            ...
            self.stdout.write(self.style.SUCCESS("Создан тестовый менеджер: username: manager, password: manager123"))
        else:
