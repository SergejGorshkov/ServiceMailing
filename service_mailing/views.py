...
class MailingListView(LoginRequiredMixin, ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["status_filter"] = self.request.GET.get(
            "status", ""
        )  # Получение статуса рассылки из GET-запроса для фильтрации в шаблоне mailing_list.html
        return context
...
class MailingDetailView(LoginRequiredMixin, DetailView):
    ...
    def get_queryset(self):
        ...
        queryset = Mailing.objects.prefetch_related("recipients", "message", "attempts", "attempts__recipient")
        # Проверка для менеджеров
        if user.has_perm("service_mailing.can_view_all_mailings"):
            return queryset
        else:
